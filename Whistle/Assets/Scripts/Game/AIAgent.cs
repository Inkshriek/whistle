...
public class AIAgent {
    ...
    //This whole class here is meant for managing artificial intelligence operations as efficiently as possible, using multithreading.
    //If you're developing AI, especially in a Behavior delegate, you should use this.
    ...
    public void GeneratePath(Vector2 start, Vector2 end) {
        ...
        Thread navParse = new Thread(() => BeginOperation(job));
        navParse.Start();
    }
    ...
    private void BeginOperation(Job job) {

        System.Diagnostics.Stopwatch sw = new System.Diagnostics.Stopwatch();
        sw.Start();

        Vector2[] path = mesh.GetPath(job.start, job.end, NavMesh.Accuracy.High);
        if (path != null) {
